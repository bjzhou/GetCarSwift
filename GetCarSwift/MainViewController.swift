...
class MainViewController: UITabBarController {
    ...
    override func viewDidLoad() {
        ...
        let track4 = RmRaceTrack(value: ["id": 4, "name": "郑州MRC赛车场", "address": "河南省郑州市惠济区新城街道郑州艺都金楸森林公园", "introduce": "郑州MRC赛车场是一条全新赛道，占地100亩，赛道全长1.3公里，宽10--15米，16个弯道，八个右弯，八个左弯。弯道设置惊险刺激，能充分展现赛车在绕弯和加速时的各项性能。同时能让选手倍感挑战性和趣味性。赛车场内设有接待大厅、餐厅和看台，是集汽车、摩托车、卡丁车、飘移、F4方程式等公路赛车的综合赛车场。同时赛车场内有国家级专业赛车教练，对车迷朋友提高驾驶技术和想进入专业车手的朋友会有非常大的帮助。", "isDeveloped": false, "mapCenter": [34.8901270000, 113.7470180000, 0], "mapZoom": 16.5])
        ...
        let track8 = RmRaceTrack(value: ["id": 8, "name": "北京金港国际赛车场", "address": "北京市朝阳区金盏乡金港大道1号", "introduce": "北京金港国际赛道是北京目前唯一的高等级汽车赛道，于2000年建成，位于北京东部的朝阳区，可容纳两万余观众。赛道由三部分构成：国际F3赛道、4×4越野赛道和安全驾驶体验场地。国际F3赛道由澳大利亚专业设计师MichaelMcDonough设计，全长2.4公里。设计最高时速为180公里/小时，平均时速120公里/小时。可容纳25辆车同时发车。连续弯道多达4处，惊险刺激、极具挑战性。", "isDeveloped": false, "mapCenter": [40.0165190369553, 116.565969380374, 0], "mapZoom": 16.5, "startLoc": [40.0162632893087, 116.567125412439, 0], "passLocs": [[40.0171486445472, 116.569112929285, 0], [40.0143302580258, 116.565381976609, 0], [40.0157014628001, 116.565194221981, 0], [40.016696724515, 116.563659998451, 0], [40.0159469414962, 116.565833928821, 0]], "leaveLoc": [40.0149475603637, 116.566123607389, 0]])
        ...
        _ = RmRaceTrack(value: ["id": 200, "name": "安吉天荒坪", "address": "安吉天荒坪", "introduce": "天荒坪位于安吉县南端，东与余杭区交界，南与临安市接壤，西连上墅乡，北接递铺镇，属西苕溪流域。全镇面积为110平方公里，下辖12个行政村，162个村民小组，6298户农户，3个居民区，总人口2.2万。2004年全镇实现工农业总产值12、24亿元，财政收入1916万元，农民人均收入6258 元。", "sightView": "tianhuangping", "mapImage": "tianhuangping_map", "isDeveloped": false, "mapCenter": [30.4600651679568, 119.599765503284, 0.0], "mapZoom": 15, "startLoc": [30.4620881289806, 119.592864948279, 873.360037027606], "stopLoc": [30.4600850062744, 119.599697063361, 792.429000178234], "cycle": false])

        gRealm?.writeOptional {
            gRealm?.add(track0, update: true)
            gRealm?.add(track1, update: true)
            gRealm?.add(track2, update: true)
            gRealm?.add(track3, update: true)
            gRealm?.add(track4, update: true)
            gRealm?.add(track5, update: true)
            gRealm?.add(track6, update: true)
            gRealm?.add(track7, update: true)
            gRealm?.add(track8, update: true)
            gRealm?.add(track9, update: true)
            gRealm?.add(track10, update: true)
            gRealm?.add(track11, update: true)
            gRealm?.add(track12, update: true)
            gRealm?.add(track13, update: true)
            gRealm?.add(track100, update: true)
            gRealm?.add(track101, update: true)
            gRealm?.add(track102, update: true)
            gRealm?.add(track103, update: true)
            gRealm?.add(track104, update: true)
            gRealm?.add(track105, update: true)
            gRealm?.add(track106, update: true)
            gRealm?.add(track107, update: true)
            gRealm?.add(track108, update: true)
            gRealm?.add(track109, update: true)
            gRealm?.add(track110, update: true)
            gRealm?.add(track111, update: true)
            gRealm?.add(track112, update: true)
            gRealm?.add(track113, update: true)
            gRealm?.add(track114, update: true)
