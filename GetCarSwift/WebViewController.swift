...
class WebViewController: UIViewController, WKNavigationDelegate {
    ...
    override func viewDidLoad() {
        ...
        progressView.frame = CGRectMake(0, 64, self.view.frame.size.width, self.progressView.frame.size.height)
        progressView.trackTintColor = UIColor(white: 1, alpha: 0)
        self.view.addSubview(progressView)
        
        backBarButton = UIBarButtonItem(image: UIImage(named: "backbutton"), style: .Plain, target: self, action: Selector("didBack"))
        forwardBarButton = UIBarButtonItem(image: UIImage(named: "forwardbutton"), style: .Plain, target: self, action: Selector("didForward"))
